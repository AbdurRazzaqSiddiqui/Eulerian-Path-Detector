    ...
    while running:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                menu_1_options = [f'{point[0]} + {point[1]}' for point in points]
                menu_2_options = [f'{point[0]} + {point[1]}' for point in points]
        ...
        if first_selection is not None:
            draw_list(menu_1_rect,f'{first_selection[0]} + {first_selection[1]}')
        else:
            draw_list(menu_1_rect,'Vertex A')
        if menu_1_expanded:
            for i, option in enumerate(menu_1_options):
        ...
        if second_selection is not None:
            draw_list(menu_2_rect,f'{second_selection[0]} + {second_selection[1]}')
        else:
